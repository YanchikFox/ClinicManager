...
public class FavoriteDoctorRepository extends AbstractDatabaseManager<FavoriteDoctor> {
    public FavoriteDoctorRepository(String dbUrl) {
        super(dbUrl);
    }
    ...
    public void delete(FavoriteDoctor fav) {
        try (PreparedStatement stmt = conn.prepareStatement("DELETE FROM favorite_doctors WHERE id = ?")) {
    ...
    public FavoriteDoctor findById(int id) {
        try (PreparedStatement stmt = conn.prepareStatement("SELECT * FROM favorite_doctors WHERE id = ?")) {
            stmt.setInt(1, id);
    ...
    public List<FavoriteDoctor> findByPatientId(int patientId) {
        List<FavoriteDoctor> list = new ArrayList<>();
        try (PreparedStatement stmt = conn.prepareStatement("SELECT * FROM favorite_doctors WHERE patient_id = ?")) {
    ...
    public void deleteByPatientAndDoctor(int patientId, int doctorId) {
        try (PreparedStatement stmt = conn.prepareStatement("DELETE FROM favorite_doctors WHERE patient_id = ? AND doctor_id = ?")) {
            stmt.setInt(1, patientId);
    ...
    public boolean isFavorite(int patientId, int doctorId) {
        try (PreparedStatement stmt = conn.prepareStatement("SELECT COUNT(*) FROM favorite_doctors WHERE patient_id = ? AND doctor_id = ?")) {
            stmt.setInt(1, patientId);
            stmt.setInt(2, doctorId);
            ResultSet rs = stmt.executeQuery();
            if (rs.next()) {
