...
public class DoctorSearchController {
  ...
  private void handleMakeAppointment() {
    ...
    if (panel == null
        || !(panel.currentPerson() instanceof com.clinicmanager.model.actors.Patient patient)) {
      new Alert(Alert.AlertType.ERROR, "Error: current patient not found", ButtonType.OK)
          .showAndWait();
      return;
    }
    ...
    if (!repositories
        .appointments()
        .canPatientBookSlot(patient.id(), selectedDoctor.id(), selectedSlot.date())) {
      new Alert(
              Alert.AlertType.WARNING,
              "You are already booked with this doctor for the selected day!",
              ButtonType.OK)
          .showAndWait();
      return;
    }
    ...
    dialog.setHeaderText(
        "Provide a short description of the problem or reason for the appointment:");
    dialog.setContentText("Description:");
    var result = dialog.showAndWait();
    if (result.isEmpty() || result.get().isBlank()) {
      new Alert(Alert.AlertType.WARNING, "You must provide a problem description!", ButtonType.OK)
          .showAndWait();
      return;
    }
  ...
  private void handleToggleFavorite() {
    ...
    if (isFav) {
      ...
      new Alert(Alert.AlertType.INFORMATION, "Doctor removed from favorites!", ButtonType.OK)
          .showAndWait();
    } else {
      repositories
          .favoriteDoctors()
          .save(new FavoriteDoctor(-1, patient.id(), selectedDoctor.id()));
      new Alert(Alert.AlertType.INFORMATION, "Doctor added to favorites!", ButtonType.OK)
          .showAndWait();
    }
    updateFavoriteBtn();
  }
