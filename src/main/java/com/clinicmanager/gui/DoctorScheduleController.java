...
public class DoctorScheduleController {
  ...
  private void setupTable() {
    slotsTable.setColumnResizePolicy(TableView.CONSTRAINED_RESIZE_POLICY_FLEX_LAST_COLUMN);
  ...
  private void loadSlotsImpl() {
    ...
    List<com.clinicmanager.model.entities.Slot> allSlots =
  ...
  private void updateSlotButtons() {
    ...
    // Closing is allowed only when the slot is free, has no patient appointments, and has not been
    // closed manually
    closeSlotBtn.setDisable(hasRealAppointment || isClosedByDoctor);
    // Opening is allowed only when the slot was closed manually (an ENDED appointment with
    // patientId = -1 exists)
    ...
    return repositories.appointments().findAll().stream()
        .anyMatch(
            a ->
                a.slotId() == slot.id()
                    && a.patientId() != -1
                    && !a.status().name().equals("CANCELLED"));
  }
  ...
  private void handleCloseSlot() {
    ...
    var app =
        new com.clinicmanager.model.entities.Appointment(
            -1, -1, doctor.id(), slot.id(), com.clinicmanager.model.enums.AppointmentStatus.ENDED);
  ...
  private Slot findSlotByRow(SlotTableRow row) {
    ...
    return repositories.slots().findAll().stream()
        .filter(
            s ->
                s.scheduleId() == doctor.scheduleId()
            ...
            slot -> {
              ...
              boolean intersects =
                  repositories.slots().findAll().stream()
                      .filter(
                          s ->
                              s.scheduleId() == doctor.scheduleId() && s.date().equals(slot.date()))
                      .anyMatch(s -> timesOverlap(s.timeRange(), slot.timeRange()));
              if (intersects) {
                new Alert(
                        Alert.AlertType.ERROR,
                        "The slot conflicts with an existing one!",
                        ButtonType.OK)
                    .showAndWait();
              } else {
  ...
  private boolean timesOverlap(
      com.clinicmanager.model.entities.TimeRange t1,
