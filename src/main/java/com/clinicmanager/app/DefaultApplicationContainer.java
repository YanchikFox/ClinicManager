...
public class DefaultApplicationContainer implements ApplicationContainer {
  ...
  public DefaultApplicationContainer(String dbUrl) {
    ...
    this.slotGenerationService =
        new SlotAutoGeneratorService(repositoryManager.doctors(), repositoryManager.slots());
    TimeManager timeManager = TimeManager.getInstance();
    this.notificationService =
        new NotificationManager(repositoryManager.notifications(), timeManager);
    this.panelManager = new DefaultPanelManager();
    this.clinic =
        new Clinic(accountService, repositoryManager.doctors(), repositoryManager.patients());
    this.timeTickHandler =
        new TimeTickHandler(
            repositoryManager.slots(), repositoryManager.appointments(), notificationService);
    timeManager.setTickListener(timeTickHandler);

    AtomicReference<ViewLoader> viewLoaderRef = new AtomicReference<>();

    this.controllerFactory =
        new FxControllerFactory(
            clinic,
            panelManager,
            repositoryManager,
            registrationService,
