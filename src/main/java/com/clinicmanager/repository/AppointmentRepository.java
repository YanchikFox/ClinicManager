...
public class AppointmentRepository extends AbstractDatabaseManager<Appointment> {
  ...
  public int save(Appointment a) {
    try (PreparedStatement stmt =
        conn.prepareStatement(
            "INSERT INTO appointments (patient_id, doctor_id, slot_id, status, problem_description)"
                + " VALUES (?, ?, ?, ?, ?)")) {
  ...
  public void update(Appointment a) {
    try (PreparedStatement stmt =
        conn.prepareStatement(
            "UPDATE appointments SET patient_id = ?, doctor_id = ?, slot_id = ?, status = ?,"
  ...
  public Appointment findById(int id) {
    try (PreparedStatement stmt =
        conn.prepareStatement("SELECT * FROM appointments WHERE id = ?")) {
      stmt.setInt(1, id);
        ...
        .noneMatch(
            a ->
                a.patientId() == patientId
                    ...
                    && !a.status()
                        .equals(com.clinicmanager.model.enums.AppointmentStatus.CANCELLED));
  }
