...
public class NotificationRepository extends AbstractDatabaseManager<Notification> {
  ...
  public Notification findById(int id) {
    try (PreparedStatement stmt =
        conn.prepareStatement("SELECT * FROM notifications WHERE id = ?")) {
      stmt.setInt(1, id);
  ...
  public List<Notification> findByPersonId(int personId) {
    List<Notification> list = new ArrayList<>();
    try (PreparedStatement stmt =
        conn.prepareStatement("SELECT * FROM notifications WHERE person_id = ?")) {
  ...
  public List<Notification> findUnreadByPersonId(int personId) {
    ...
    try (PreparedStatement stmt =
        conn.prepareStatement("SELECT * FROM notifications WHERE person_id = ? AND read = 0")) {
      stmt.setInt(1, personId);
      ResultSet rs = stmt.executeQuery();
      while (rs.next()) {
        result.add(
