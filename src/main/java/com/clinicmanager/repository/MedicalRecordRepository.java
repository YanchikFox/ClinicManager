...
public class MedicalRecordRepository extends AbstractDatabaseManager<MedicalRecord> {
  ...
  public int save(MedicalRecord record) {
    try (PreparedStatement stmt =
        conn.prepareStatement(
            "INSERT INTO medical_records (medical_card_id, doctor_id, date, description,"
  ...
  public void delete(MedicalRecord record) {
    try (PreparedStatement stmt =
        conn.prepareStatement("DELETE FROM medical_records WHERE id = ?")) {
  ...
  public void update(MedicalRecord record) {
    try (PreparedStatement stmt =
        conn.prepareStatement(
            "UPDATE medical_records SET medical_card_id = ?, doctor_id = ?, date = ?, description ="
  ...
  public MedicalRecord findById(int id) {
    try (PreparedStatement stmt =
        conn.prepareStatement("SELECT * FROM medical_records WHERE id = ?")) {
      stmt.setInt(1, id);
  ...
  public List<MedicalRecord> findByMedicalCardId(int medicalCardId) {
    ...
    try (PreparedStatement stmt =
        conn.prepareStatement("SELECT * FROM medical_records WHERE medical_card_id = ?")) {
  ...
  public MedicalRecord findByAppointmentId(int appointmentId) {
    try (PreparedStatement stmt =
        conn.prepareStatement("SELECT * FROM medical_records WHERE appointment_id = ?")) {
  ...
  public boolean existsForAppointment(int appointmentId) {
    try (PreparedStatement stmt =
        conn.prepareStatement("SELECT 1 FROM medical_records WHERE appointment_id = ?")) {
      stmt.setInt(1, appointmentId);
      ResultSet rs = stmt.executeQuery();
      return rs.next();
    } catch (SQLException e) {
