...
public class ScheduleRepository extends AbstractDatabaseManager<Schedule> {
  ...
  public int save(Schedule s) {
    try (PreparedStatement stmt =
        conn.prepareStatement("INSERT INTO schedules (doctor_id) VALUES (?)")) {
  ...
  public void update(Schedule s) {
    try (PreparedStatement stmt =
        conn.prepareStatement("UPDATE schedules SET doctor_id = ? WHERE id = ?")) {
      stmt.setInt(1, s.doctorId());
      stmt.setInt(2, s.id());
      stmt.executeUpdate();
    } catch (SQLException e) {
  ...
  public Schedule findByDoctorId(int doctorId) {
    try (PreparedStatement stmt =
        conn.prepareStatement("SELECT * FROM schedules WHERE doctor_id = ? LIMIT 1")) {
  ...
  public List<Slot> getOutgoingSchedule(int scheduleId) {
    List<Slot> slots = new ArrayList<>();
    String sql =
        "SELECT * FROM slots WHERE schedule_id = ? AND date >= ? ORDER BY date, start_time";
    try (PreparedStatement stmt = conn.prepareStatement(sql)) {
      stmt.setInt(1, scheduleId);
